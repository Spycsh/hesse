...
/**
 * partition by vertexId storage function each vertex storage function instance will be distributed
 * to different Flink StateFun workers by using hash partitioning on its address and the
...
public class VertexStorageFn implements StatefulFunction {
  ...
  /**
   * A pre-config event time interval that used to separate the whole sequence of temporal edges
   * into batches e.g. there are three records with timestamp 0, 5, 7 and the event time interval is
  ...
  static {
    ...
    VertexStorageFn.storeIngoingEdges =
        Boolean.parseBoolean(prop.getProperty("STORE_INGOING_EDGES"));
  }

  // valueSpecs of outer vertex activities
  private static final ValueSpec<List<VertexActivity>> VERTEX_ACTIVITIES_LIST =
      ValueSpec.named("vertexActivitiesList").withCustomType(Types.VERTEX_ACTIVITIES_LIST_TYPE);
  private static final ValueSpec<PriorityQueue<VertexActivity>> VERTEX_ACTIVITIES_PQ =
      ValueSpec.named("vertexActivitiesPQ").withCustomType(Types.VERTEX_ACTIVITIES_PQ_TYPE);
  ...
  static {
    if (VertexStorageFn.storageParadigm == 4) {
      for (int i = 0; i < VertexStorageFn.bucketSize; i++) {
        bucketMap.put(
            String.valueOf(i), ValueSpec.named("bucket" + i).withCustomType(Types.BUCKET_TYPE));
      }
    }
  }
  ...
  public CompletableFuture<Void> apply(Context context, Message message) throws Throwable {
    ...
    if (message.is(Types.TEMPORAL_EDGE_TYPE)) {
      ...
      context.send(
          MessageBuilder.forAddress(
                  TypeName.typeNameOf("hesse.benchmarks", "benchmark-storage-time"), "1")
              .withValue(storageEndTime - storageStartTime)
              .build());

      // send all the nodes ids to Coordinator 0
      context.send(
          MessageBuilder.forAddress(TypeName.typeNameOf("hesse.coordination", "coordinator"), "0")
    ...
    if (message.is(Types.QUERY_GNN_SAMPLING_TYPE)) {
      ...
      LOGGER.info(
          "[VertexStorageFn {}] QueryGNNSampling received with startT:{}, endT:{}",
          context.self().id(),
          q.getStartT(),
          q.getEndT());
      ...
      List<VertexActivity> filteredActivityList =
          filterActivityListByTimeRegion(context, q.getStartT(), q.getEndT());

      QueryGNNSamplingState queryWithState = new QueryGNNSamplingState(q, filteredActivityList);
    ...
    if (message.is(Types.QUERY_SCC_TYPE)) {
      ...
      LOGGER.info(
          "[VertexStorageFn {}] QueryStronglyConnectedComponent received with startT:{}, endT:{}",
          context.self().id(),
          q.getStartT(),
          q.getEndT());
      ...
      List<VertexActivity> filteredActivityList =
          filterActivityListByTimeRegion(context, q.getStartT(), q.getEndT());
      QuerySCCWithState queryWithState = new QuerySCCWithState(q, filteredActivityList);
    ...
    if (message.is(Types.QUERY_CC_TYPE)) {
      ...
      LOGGER.info(
          "[VertexStorageFn {}] QueryConnectedComponent received with startT:{}, endT:{}",
          context.self().id(),
          q.getStartT(),
          q.getEndT());
      ...
      List<VertexActivity> filteredActivityList =
          filterActivityListByTimeRegion(context, q.getStartT(), q.getEndT());
      QueryCCWithState queryWithState = new QueryCCWithState(q, filteredActivityList);
    ...
    if (message.is(Types.QUERY_SSSP_TYPE)) {
      ...
      LOGGER.info(
          "[VertexStorageFn {}] QuerySingleSourceShortestPath received with startT:{}, endT:{}",
          context.self().id(),
          q.getStartT(),
          q.getEndT());
      ...
      List<VertexActivity> filteredActivityList =
          filterActivityListByTimeRegion(context, q.getStartT(), q.getEndT());
      QuerySSSPWithState queryWithState = new QuerySSSPWithState(q, filteredActivityList);
    ...
    if (message.is(Types.PAGERANK_PREPARE_TASK_TYPE)) {
      ...
      LOGGER.info(
          "[VertexStorageFn {}] PageRankTask received with startT:{}, endT:{}",
          context.self().id(),
          q.getStartT(),
          q.getEndT());
      ...
      List<VertexActivity> filteredActivityList =
          filterActivityListByTimeRegion(context, q.getStartT(), q.getEndT());
      PageRankTaskWithState taskWithState = new PageRankTaskWithState(q, filteredActivityList);
    ...
    if (message.is(Types.FORWARD_QUERY_GNN_SAMPLING_TYPE)) {
      ...
      LOGGER.info(
          "[VertexStorageFn {}] ForwardQueryGNNSampling received with startT:{}, endT:{}",
          context.self().id(),
          q.getStartT(),
          q.getEndT());
      ...
      List<VertexActivity> filteredActivityList =
          filterActivityListByTimeRegion(context, q.getStartT(), q.getEndT());
      ForwardQueryGNNSamplingWithState queryWithState =
          new ForwardQueryGNNSamplingWithState(q, filteredActivityList);
      sendQueryGNNSamplingWithStateToApp(context, queryWithState);
    }
    ...
    if (message.is(Types.FORWARD_QUERY_SCC_TYPE)) {
      ...
      List<VertexActivity> filteredActivityList =
          filterActivityListByTimeRegion(context, q.getStartT(), q.getEndT());
      ForwardQuerySCCWithState queryWithState =
          new ForwardQuerySCCWithState(q, filteredActivityList);
      sendQueryStronglyConnectedComponentWithStateToApp(context, queryWithState);
    }
    ...
    if (message.is(Types.FORWARD_QUERY_CC_TYPE)) {
      ...
      List<VertexActivity> filteredActivityList =
          filterActivityListByTimeRegion(context, q.getStartT(), q.getEndT());
      ForwardQueryCCWithState queryWithState = new ForwardQueryCCWithState(q, filteredActivityList);
      sendQueryConnectedComponentWithStateToApp(context, queryWithState);
    }
  ...
  private void storeIngoingActivity(Context context, VertexActivity activity) {
    context.send(
        MessageBuilder.forAddress(
                TypeName.typeNameOf("hesse.storage", "vertex-storage"), activity.getDstId())
            .withCustomType(Types.VERTEX_ACTIVITY_TYPE, activity)
            .build());
  }

  private List<VertexActivity> filterActivityListByTimeRegion(
      Context context, int startT, int endT) {
    long filterStartTime = System.nanoTime();
    List<VertexActivity> vertexActivitiesRes = new LinkedList<>();
    switch (storageParadigm) {
      case 1:
        {
          List<VertexActivity> vertexActivitiesList =
              context.storage().get(VERTEX_ACTIVITIES_LIST).orElse(new LinkedList<>());
          for (VertexActivity curActivity : vertexActivitiesList) {
            if (Integer.parseInt(curActivity.getTimestamp()) <= endT
                && Integer.parseInt(curActivity.getTimestamp()) >= startT) {
      ...
      case 2:
        {
          PriorityQueue<VertexActivity> vertexActivitiesPQ =
              context.storage().get(VERTEX_ACTIVITIES_PQ).orElse(new PriorityQueue<>());
      ...
      case 3:
        {
          ...
          vertexActivitiesRes.removeIf(
              v ->
                  Integer.parseInt(v.getTimestamp()) < startT
                      || Integer.parseInt(v.getTimestamp()) > endT);
          break;
        }
      case 4:
        {
          ...
          for (int i = batchIndexStart; i <= batchIndexEnd; i++) {
            if (bucketMap.containsKey(String.valueOf(i))) {
              List<VertexActivity> list =
                  context
                      .storage()
                      .get(bucketMap.get(String.valueOf(i)))
      ...
      default:
        {
          throw new IllegalArgumentException(
              "[VertexStorageFn] Wrong storage paradigm index, please check property file!");
        }
    }
    ...
    context.send(
        MessageBuilder.forAddress(
                TypeName.typeNameOf("hesse.benchmarks", "benchmark-filter-time"), "1")
      ...
      case 3:
        {
          ...
          List<VertexActivity> batchActivity =
              vertexActivitiesBRBT.getOrDefault(batchIndex, new LinkedList<>());
      ...
      case 4:
        {
          ...
          List<VertexActivity> vertexActivities =
              context.storage().get(bucketMap.get(bucketIndex)).orElse(new LinkedList<>());
  ...
  private void sendQueryGNNSamplingWithStateToApp(
      Context context, QueryGNNSamplingState queryWithState) {
    context.send(
        MessageBuilder.forAddress(
                TypeName.typeNameOf("hesse.applications", "gnn-sampling"), context.self().id())
            .withCustomType(Types.QUERY_GNN_SAMPLING_WITH_STATE_TYPE, queryWithState)
            .build());
  }

  private void sendQueryGNNSamplingWithStateToApp(
      Context context, ForwardQueryGNNSamplingWithState queryWithState) {
    context.send(
        MessageBuilder.forAddress(
                TypeName.typeNameOf("hesse.applications", "gnn-sampling"), context.self().id())
            .withCustomType(Types.FORWARD_QUERY_GNN_SAMPLING_WITH_STATE_TYPE, queryWithState)
            .build());
  }

  private void sendQueryStronglyConnectedComponentWithStateToApp(
      Context context, QuerySCCWithState queryWithState) {
    context.send(
        MessageBuilder.forAddress(
                TypeName.typeNameOf("hesse.applications", "strongly-connected-components"),
                context.self().id())
            .withCustomType(Types.QUERY_SCC_WITH_STATE_TYPE, queryWithState)
            .build());
  }

  private void sendQueryStronglyConnectedComponentWithStateToApp(
      Context context, ForwardQuerySCCWithState queryWithState) {
    context.send(
        MessageBuilder.forAddress(
                TypeName.typeNameOf("hesse.applications", "strongly-connected-components"),
                context.self().id())
            .withCustomType(Types.FORWARD_QUERY_SCC_WITH_STATE_TYPE, queryWithState)
            .build());
  }

  private void sendQueryConnectedComponentWithStateToApp(
      Context context, QueryCCWithState queryWithState) {
    context.send(
        MessageBuilder.forAddress(
                TypeName.typeNameOf("hesse.applications", "connected-components"),
                context.self().id())
            .withCustomType(Types.QUERY_CC_WITH_STATE_TYPE, queryWithState)
            .build());
  }

  private void sendQueryConnectedComponentWithStateToApp(
      Context context, ForwardQueryCCWithState queryWithState) {
    context.send(
        MessageBuilder.forAddress(
                TypeName.typeNameOf("hesse.applications", "connected-components"),
                context.self().id())
            .withCustomType(Types.FORWARD_QUERY_CC_WITH_STATE_TYPE, queryWithState)
            .build());
  }

  private void sendQuerySingleSourceShortestPathWithStateToApp(
      Context context, QuerySSSPWithState queryWithState) {
    context.send(
        MessageBuilder.forAddress(
                TypeName.typeNameOf("hesse.applications", "single-source-shortest-path"),
                context.self().id())
            .withCustomType(Types.QUERY_SSSP_WITH_STATE_TYPE, queryWithState)
            .build());
  }
  ...
  private void sendQueryStateToApp(Context context, QueryState state) {
    context.send(
        MessageBuilder.forAddress(
                TypeName.typeNameOf("hesse.applications", "single-source-shortest-path"),
                context.self().id())
            .withCustomType(Types.QUERY_STATE_TYPE, state)
            .build());
  }

  private void sendPageRankTaskWithStateToApp(
      Context context, PageRankTaskWithState taskWithState) {
    context.send(
        MessageBuilder.forAddress(
                TypeName.typeNameOf("hesse.applications", "pagerank"), context.self().id())
