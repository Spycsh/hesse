...
/**
 * this function serves the temporal query of the graph about the GNNSampling, i.e. K-hop H-size
 * neighbourhood of one node in any arbitrary time window refer to
 * http://snap.stanford.edu/graphsage/sample_and_agg.png for more details the question can be
 * simplified as following: given H and K and t H: sample size for each hop K: hop number t:
 * timestamp return the K-hop H-size neighbourhood of one node
 */
public class GNNSamplingFn implements StatefulFunction {

  private static final ValueSpec<ArrayList<QueryGNNSamplingContext>>
      QUERY_GNN_SAMPLING_CONTEXT_LIST =
          ValueSpec.named("queryGNNSamplingContextList")
              .withCustomType(Types.QUERY_GNN_SAMPLING_CONTEXT_LIST_TYPE);
  ...
  public CompletableFuture<Void> apply(Context context, Message message) throws Throwable {
    ...
    if (message.is(Types.QUERY_GNN_SAMPLING_WITH_STATE_TYPE)) {
      ...
      if (neighbourIds.size() == 0) {
        ...
        String str1 =
            String.format(
                "Result of query %s by user %s: no results found for node %s because it is isolated!",
        ...
        // it should collect H responses (when H is less than neighbour size, collect the neighbour
        // size)
        int responseNumToCollect = Math.min(H, neighbourIds.size());

        GNNSamplingPathContext GNNSamplingPathContext =
            new GNNSamplingPathContext(
                Utils.generateNewStackHash(firstStk), responseNumToCollect, new ArrayList<>());
        queryGNNSamplingContextList.add(
            new QueryGNNSamplingContext(
                q.getQueryId(),
                q.getUserId(),
                new ArrayList<GNNSamplingPathContext>() {
                  {
          ...
          for (String neighbourId : neighbourIdList) {
            ...
            context.send(
                MessageBuilder.forAddress(
                        TypeName.typeNameOf("hesse.storage", "vertex-storage"), neighbourId)
                    .withCustomType(
                        Types.FORWARD_QUERY_GNN_SAMPLING_TYPE,
                        new ForwardQueryGNNSampling(
                            context.self().id(), neighbourId, q, q.getK() - 1, firstStk))
                    .build());
          }
        }
      }
    ...
    if (message.is(Types.FORWARD_QUERY_GNN_SAMPLING_WITH_STATE_TYPE)) {
      LOGGER.debug(
          "[GNNSamplingFn {}] ForwardQueryGNNSamplingWithState received", context.self().id());

      ForwardQueryGNNSamplingWithState q =
          message.as(Types.FORWARD_QUERY_GNN_SAMPLING_WITH_STATE_TYPE);
      int K = q.getK();
      ...
      if (K == 0 || neighbourIds.size() == 0) {
        ...
        if (K == 0) {
          LOGGER.debug(
              "[GNNSamplingFn {}] ForwardQueryGNNSamplingWithState K is 0, send back the queryGNNSamplingResult",
              context.self().id());
        } else {
          LOGGER.debug(
              "[GNNSamplingFn {}] ForwardQueryGNNSamplingWithState neighbour size is 0, send back the queryGNNSamplingResult",
              context.self().id());
        }

        ArrayList<Edge> aggregatedResults = new ArrayList<>();
        aggregatedResults.add(new Edge(sourceId, context.self().id()));
        QueryGNNSamplingResult queryGNNSamplingResult =
            new QueryGNNSamplingResult(
                q.getQueryId(),
                q.getUserId(),
                q.getVertexId(),
                q.getQueryType(),
        ...
        context.send(
            MessageBuilder.forAddress(
                    TypeName.typeNameOf("hesse.applications", "gnn-sampling"), sourceId)
                .withCustomType(Types.QUERY_GNN_SAMPLING_RESULT_TYPE, queryGNNSamplingResult)
                .build());
      } else if (K > 0) {
        LOGGER.debug(
            "[GNNSamplingFn {}] ForwardQueryGNNSamplingWithState K > 0, continue forwarding to neighbours",
            context.self().id());
        ...
        for (String neighbourId : neighbourIdList) {
          ...
          LOGGER.debug(
              "[GNNSamplingFn {}] forwarding to neighbour {}...", context.self().id(), neighbourId);
          ...
          context.send(
              MessageBuilder.forAddress(
                  ...
                  .withCustomType(
                      Types.FORWARD_QUERY_GNN_SAMPLING_TYPE,
                      new ForwardQueryGNNSampling(
                          context.self().id(), neighbourId, q, K - 1, stack))
                  .build());
        }
        // find the queryGNNSamplingContext with the queryId and userId
        // set the source ids and the response number
        QueryGNNSamplingContext e =
            findQueryGNNSamplingContext(q.getQueryId(), q.getUserId(), queryGNNSamplingContextList);
        ...
        if (e == null) {
          ...
          list.add(
              new GNNSamplingPathContext(
                  Utils.generateNewStackHash(currentStack),
                  responseNumToCollect,
                  new ArrayList<>()));
          QueryGNNSamplingContext qc =
              new QueryGNNSamplingContext(q.getQueryId(), q.getUserId(), list);
          queryGNNSamplingContextList.add(qc);
        } else {
          e.getGNNSamplingPathContexts()
              .add(
                  new GNNSamplingPathContext(
                      Utils.generateNewStackHash(currentStack),
                      responseNumToCollect,
    ...
    if (message.is(Types.QUERY_GNN_SAMPLING_RESULT_TYPE)) {
      ...
      ArrayList<QueryGNNSamplingContext> queryGNNSamplingContextList =
          context.storage().get(QUERY_GNN_SAMPLING_CONTEXT_LIST).orElse(new ArrayList<>());
      QueryGNNSamplingContext queryGNNSamplingContext =
          findQueryGNNSamplingContext(
              result.getQueryId(), result.getUserId(), queryGNNSamplingContextList);
      ...
      ArrayList<GNNSamplingPathContext> GNNSamplingPathContext =
          queryGNNSamplingContext.getGNNSamplingPathContexts();
      GNNSamplingPathContext gnnSamplingContextByPathHash =
          findGNNSamplingContextByStackHash(GNNSamplingPathContext, stackHash);
      ...
      if (gnnSamplingContextByPathHash.getResponseNum() > 1) {
        LOGGER.debug(
            "[GNNSamplingFn {}] queryGNNSamplingContext not collects all the results, still {} "
                + " result(s) to collect",
            context.self().id(),
            gnnSamplingContextByPathHash.getResponseNum() - 1);

        // after collecting this result, still not collect all results,
        // so just aggregate the new result, not send or egress, because not received all the
        // excepted results
        ...
        gnnSamplingContextByPathHash.setResponseNum(
            gnnSamplingContextByPathHash.getResponseNum() - 1);
      } else {
        ...
        if (context.self().id().equals(result.getVertexId())) {
          ...
          result
              ...
              .addAll(gnnSamplingContextByPathHash.getAggregatedGNNSamplingEdges());

          StringBuilder sb = new StringBuilder();
          sb.append(
              String.format(
                  "Result of query %s by user %s, edges including in the gnn-sampling are: ",
                  result.getQueryId(), result.getUserId()));
          for (Edge e : result.getAggregatedResults())
          ...
          LOGGER.debug(
              "[GNNSamplingFn {}] queryGNNSamplingContext collects all the results but not the source",
              context.self().id());
          ...
          context.send(
              MessageBuilder.forAddress(
                      TypeName.typeNameOf("hesse.applications", "gnn-sampling"), stack.getFirst())
                  .withCustomType(Types.QUERY_GNN_SAMPLING_RESULT_TYPE, result)
                  .build());
        }
  ...
  private GNNSamplingPathContext findGNNSamplingContextByStackHash(
      ArrayList<GNNSamplingPathContext> GNNSamplingPathContext, int stackHash) {
  ...
  private QueryGNNSamplingContext findQueryGNNSamplingContext(
      String queryId,
      String userId,
      ArrayList<QueryGNNSamplingContext> queryGNNSamplingContextList) {
  ...
  private void sendResult(Context context, String queryId, String userId, String resultStr) {
    context.send(
        MessageBuilder.forAddress(
                TypeName.typeNameOf("hesse.query", "temporal-query-handler"), queryId)
            .withCustomType(Types.QUERY_RESULT_TYPE, new QueryResult(queryId, userId, resultStr))
            .build());
  }
}
