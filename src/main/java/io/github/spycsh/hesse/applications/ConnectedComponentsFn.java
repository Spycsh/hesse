...
/**
 * this function serves the temporal query of the vertex's connected component id and all ids in the
 * same connected component in any arbitrary time window
 */
public class ConnectedComponentsFn implements StatefulFunction {
  ...
  private static final org.slf4j.Logger LOGGER =
      LoggerFactory.getLogger(ConnectedComponentsFn.class);
  ...
  public CompletableFuture<Void> apply(Context context, Message message) throws Throwable {
      ...
      if (neighbourIds.size() == 0) {
        ...
        QueryCCContext c =
            new QueryCCContext(
                ...
                new HashSet<>(),
                neighbourIds.size());
        ArrayList<QueryCCContext> list =
            context.storage().get(QUERY_CC_CONTEXT_LIST).orElse(new ArrayList<>());
        list.add(c);
    ...
    if (message.is(Types.FORWARD_QUERY_CC_WITH_STATE_TYPE)) {
      LOGGER.debug(
          "[ConnectedComponentsFn {}] ForwardQueryCCWithState received", context.self().id());
      ...
      context.send(
          MessageBuilder.forAddress(
              ...
              .withCustomType(
                  ...
                  new QueryCCNeighbours(
                      q.getQueryId(), q.getUserId(), q.getVertexId(), neighbourIds))
              .build());
    }
    ...
    if (message.is(Types.QUERY_CC_NEIGHBOURS_TYPE)) {
      ...
      if (ccContext.getCandidateResponseNumber() == 0) {
        if (currentCandidateNeighbours.size() == 0) {
          ...
          String str1 =
              String.format(
                  "Result of query %s by user %s: connected component id of node %s is %s. ",
                  q.getQueryId(), q.getUserId(), context.self().id(), lowLinkId);
          String str2 =
              "All node ids that contain in the same component are:" + vertexSet.toString();
  ...
  private QueryCCContext findCCContext(
      String queryId, String userId, ArrayList<QueryCCContext> list) {
