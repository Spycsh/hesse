...
/**
 * this function serves the temporal query of the graph about the strongly connected component e.g.
 * it can answer the vertex's strongly connected component id and all ids including in the SCC in
...
public class StronglyConnectedComponentsFn implements StatefulFunction {
  ...
  private static final org.slf4j.Logger LOGGER =
      LoggerFactory.getLogger(StronglyConnectedComponentsFn.class);

  @Override
  public CompletableFuture<Void> apply(Context context, Message message) throws Throwable {
    if (message.is(Types.QUERY_SCC_WITH_STATE_TYPE)) {
      LOGGER.debug(
          "[StronglyConnectedComponentsFn {}] QuerySCCWithState received", context.self().id());
      ...
      if (neighbourIds.size() == 0) {
        ...
        String str1 =
            String.format(
                "Result of query %s by user %s: no connected component found for node %s because it is isolated!",
                q.getQueryId(), q.getUserId(), context.self().id());
        result.append(str1);
        sendResult(context, q.getQueryId(), q.getUserId(), result.toString());
      } else {

        ArrayList<QuerySCCContext> querySCCContexts =
            context.storage().get(QUERY_SCC_CONTEXT_LIST).orElse(new ArrayList<>());
        /*
        ...
        */
        ...
        SCCPathContext sccPathContext =
            new SCCPathContext(
                generateNewStackHash(firstStk),
                context.self().id(),
                false,
                neighbourIds.size(),
                new HashSet<>());
        querySCCContexts.add(
            new QuerySCCContext(
                q.getQueryId(),
                q.getUserId(),
    ...
    if (message.is(Types.FORWARD_QUERY_SCC_WITH_STATE_TYPE)) {
      LOGGER.debug(
          "[StronglyConnectedComponentsFn {}] ForwardQuerySCCWithState received",
          context.self().id());
      ...
      ArrayList<QuerySCCContext> querySCCContexts =
          context.storage().get(QUERY_SCC_CONTEXT_LIST).orElse(new ArrayList<>());
      QuerySCCContext querySCCContext =
          findSCCContext(q.getQueryId(), q.getUserId(), querySCCContexts);

      // if there is a cycle back to the queried vertex, then there is a strongly connected
      // component
      // do backtracking
      if (q.getVertexId().equals(context.self().id())) {
        LOGGER.debug(
            "[StronglyConnectedComponentsFn {}] ForwardQuerySCCWithState received and there is"
        ...
        if (querySCCContext == null) {
          throw new IllegalStateException(
              "querySCCContext "
                  + context.self().id()
                  + " should not be null because it is on the stack");
        } else {
          context.send(
              MessageBuilder.forAddress(
                      TypeName.typeNameOf("hesse.applications", "strongly-connected-components"),
      ...
      } else if (stack.contains(context.self().id())
          && checkConsecutiveCycle(new ArrayDeque<>(stack))) {
        /*
        ...
        */
        ...
        LOGGER.debug(
            "[StronglyConnectedComponentsFn {}] ForwardQuerySCCWithState received and "
                + "fail to find a SCC",
            context.self().id());
        ...
        if (querySCCContext == null) {
          SCCPathContext sccPathContext =
              new SCCPathContext(
                  newStackHash, context.self().id(), false, neighbourIds.size(), new HashSet<>());
          querySCCContext =
              new QuerySCCContext(
                  q.getQueryId(),
                  q.getUserId(),
    ...
    if (message.is(Types.QUERY_SCC_RESULT_TYPE)) {
      LOGGER.debug(
          "[StronglyConnectedComponentsFn {}}] QuerySCCResult received", context.self().id());
      QuerySCCResult result = message.as(Types.QUERY_SCC_RESULT_TYPE);

      ArrayList<QuerySCCContext> querySCCContexts =
          context.storage().get(QUERY_SCC_CONTEXT_LIST).orElse(new ArrayList<>());
      QuerySCCContext querySCCContext =
          findSCCContext(result.getQueryId(), result.getUserId(), querySCCContexts);
      if (querySCCContext == null) {
        LOGGER.error(
            "querySCCContext should not be null because the result is sent back from children nodes");
        throw new IllegalStateException(
            "querySCCContext should not be null because the result is sent back from children nodes\n");
      }
      ...
      if (sccContextByPathHash == null) {
        LOGGER.error(
            "sccContextByPathHash should not be null because the result is sent back from children nodes");
        throw new IllegalStateException(
            "sccContextByPathHash should not be null because the result is sent back from children nodes\n");
      }
      ...
      if (n - 1 == 0) {
        LOGGER.debug(
            "[StronglyConnectedComponentsFn {}] QuerySCCResult received "
        ...
        if (context.self().id().equals(result.getVertexId())) {
          LOGGER.info(
              "[StronglyConnectedComponentsFn {}] Success! qid: {}, uid: {}",
              context.self().id(),
              result.getQueryId(),
              result.getUserId());
          String str1 =
              String.format(
                  "Result of query %s by user %s: Strongly connected component id of node %s is %s. ",
          ...
          for (String id : aggregatedSCCIds) sb.append(" ").append(id);
          System.out.println();
          ...
          LOGGER.debug(
              "[StronglyConnectedComponentsFn {}] not the source node", context.self().id());
              ...
              MessageBuilder.forAddress(
                  ...
                  .withCustomType(
                      ...
                      new QuerySCCResult(
                          ...
                          result.getVertexId(),
                          result.getQueryType(),
                          String.valueOf(updatedLowLinkId),
                          sccFlag,
                          aggregatedSCCIds,
                          stack))
                  .build());
  ...
  private SCCPathContext findSCCContextByPathHash(
      ArrayList<SCCPathContext> sccPathContexts, int stackHash) {
    for (SCCPathContext c : sccPathContexts) {
  ...
  private QuerySCCContext findSCCContext(
      String queryId, String userId, ArrayList<QuerySCCContext> list) {
