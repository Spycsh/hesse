...
/**
 * this function serves the temporal query of the graph about the single source shortest path of all
 * other approachable vertices to the given vertex in any arbitrary time window
 */
public class SingleSourceShortestPathFn implements StatefulFunction {
  ...
  private static final org.slf4j.Logger LOGGER =
      LoggerFactory.getLogger(SingleSourceShortestPathFn.class);
  ...
  public CompletableFuture<Void> apply(Context context, Message message) throws Throwable {
    ...
    if (message.is(Types.QUERY_SSSP_WITH_STATE_TYPE)) {
      LOGGER.debug(
          "[SingleSourceShortestPathFn {}] QuerySSSPWithState received", context.self().id());
      ...
      Map<String, String> neighbourIdsWithWeight =
          Utils.recoverWeightedStateByLog(vertexActivities);
      ...
      if (neighbourIdsWithWeight.size() == 0) {
        ...
        String str1 =
            String.format(
                "Result of query %s by user %s: no shortest path from node %s because there are no outgoing edge from it!",
        ...
        ArrayList<QuerySSSPContext> querySSSPContexts =
            context.storage().get(QUERY_SSSP_CONTEXT_LIST).orElse(new ArrayList<>());

        neighbourIdsWithWeight.put(context.self().id(), "0.0");
        querySSSPContexts.add(
            new QuerySSSPContext(
                q.getQueryId(),
                q.getUserId(),
                q.getStartT(),
                q.getEndT(),
                vertexSet,
                neighbourIdsWithWeight,
                paths));
        ...
        context.send(
            MessageBuilder.forAddress(
                    TypeName.typeNameOf("hesse.storage", "vertex-storage"), minimumWeightIdStr)
                .withCustomType(
                    Types.QUERY_STATE_REQUEST_TYPE,
                    new QueryStateRequest(
                        q.getQueryId(), q.getUserId(), q.getVertexId(), q.getStartT(), q.getEndT()))
                .build());
      }
    }
    ...
    if (message.is(Types.QUERY_STATE_TYPE)) {
      ...
      Map<String, String> neighbourIdsWithWeight =
          Utils.recoverWeightedStateByLog(q.getVertexActivities());
    ...
    if (message.is(Types.QUERY_NEXT_NEIGHBOURS_INFO)) {
      LOGGER.debug(
          "[SingleSourceShortestPathFn {}] QueryNextNeighbourInfo received", context.self().id());
      ...
      ArrayList<QuerySSSPContext> querySSSPContexts =
          context.storage().get(QUERY_SSSP_CONTEXT_LIST).orElse(new ArrayList<>());
      QuerySSSPContext querySSSPContext =
          findSSSPContext(q.getQueryId(), q.getUserId(), querySSSPContexts);

      if (querySSSPContext == null) {
        LOGGER.error(
            "[SingleSourceShortestPathFn {}] querySSSPContext should not be null!",
            context.self().id());
      ...
      for (Map.Entry<String, String> e : nextNeighbourIdsWithWeight.entrySet()) {
        String id1 = e.getKey();
        double newDistToNextNeighbour =
            Double.parseDouble(distToNeighbour) + Double.parseDouble(e.getValue());
      ...
      // if all neighbour ids equals to vertexSet, it means all the approachable nodes find shortest
      // path, egress
      if (neighbourIdsWithWeight.keySet().equals(vertexSet)) {
        StringBuilder result = new StringBuilder();
        String str1 =
            String.format(
                "Result of query %s by user %s: shortest path(s) from node %s to others: ",
                q.getQueryId(), q.getUserId(), context.self().id());
        result.append(str1);
        for (Map.Entry<String, String> e : neighbourIdsWithWeight.entrySet()) {
          String id = e.getKey();
        ...
        // find the shortest path vertex among those not in the vertexSet and send to it the state
        // request
        for (Map.Entry<String, String> e : neighbourIdsWithWeight.entrySet()) {
          String id = e.getKey();
            ...
            MessageBuilder.forAddress(
                ...
                .withCustomType(
                    ...
                    new QueryStateRequest(
                        ...
                        q.getUserId(),
                        q.getVertexId(),
                        querySSSPContext.getStartT(),
                        querySSSPContext.getEndT()))
                .build());
      }
    }
  ...
  private QuerySSSPContext findSSSPContext(
      String queryId, String userId, ArrayList<QuerySSSPContext> list) {
