...
/**
 * this function, along with the coordinator function, serve the temporal query of the graph about
 * the PageRank e.g. it can answer all the vertices' PageRank values in a time window for each
 * iteration
 */
public class PageRankFn implements StatefulFunction {
  ...
  public CompletableFuture<Void> apply(Context context, Message message) throws Throwable {
    ...
    // it needs to recover its in-degree and neighbours by using the state and store in context for
    // next computation
    if (message.is(Types.PAGERANK_TASK_WITH_STATE_TYPE)) {
      ...
      LOGGER.debug(
          "[PageRankFn {}] init pagerank context with qid: {}, uid: {}, ",
          context.self().id(),
          q.getQueryId(),
          q.getUserId());

      ArrayList<PageRankContext> pageRankContexts =
          context.storage().get(PAGE_RANK_CONTEXT_LIST).orElse(new ArrayList<>());

      if (neighbourIdsWithWeight.size() == 0 && inDegree == 0) {
        pageRankContexts.add(
            new PageRankContext(
                q.getQueryId(),
                q.getUserId(),
                q.getStartT(),
    ...
    if (message.is(Types.PAGERANK_VALUE_WITH_WEIGHT_TYPE)) {
      ...
      LOGGER.debug(
          "[PageRankFn {}] PageRankValueWithWeight received PR:{}, w:{}, W:{}",
          context.self().id(),
          p.getPagerankValue(),
          p.getWeight(),
          p.getWeightSum());
      ArrayList<PageRankContext> pageRankContexts =
          context.storage().get(PAGE_RANK_CONTEXT_LIST).orElse(new ArrayList<>());
      PageRankContext pageRankContext =
          findPageRankContext(p.getQueryId(), p.getUserId(), pageRankContexts);
      if (pageRankContext == null) {
        LOGGER.error(
            "[PageRankFn {}] PageRankContext does not exist! qid: {}, uid: {}",
            context.self().id(),
            p.getQueryId(),
            p.getUserId());
      }
      ...
      collectedDegree += 1;
      LOGGER.debug(
          "[PageRankFn {}] in-degree: {}, current collected: {}",
          context.self().id(),
          pageRankContext.getInDegree(),
          collectedDegree);
      ...
      if (collectedDegree == pageRankContext.getInDegree()) {
        ...
        context.send(
            MessageBuilder.forAddress(
                    ...
                    pageRankContext.getCoordinatorId())
                .withCustomType(
                    Types.QUERY_PAGERANK_RESULT_TYPE,
                    new QueryPageRankResult(
                        p.getQueryId(),
                        p.getUserId(),
                        context.self().id(),
    ...
    // in this following rounds, each PageRank continue the computation with its in-degree and
    // neighbours
    if (message.is(Types.PAGERANK_CONTINUE_TASK_TYPE)) {
      ...
      ArrayList<PageRankContext> pageRankContexts =
          context.storage().get(PAGE_RANK_CONTEXT_LIST).orElse(new ArrayList<>());
      PageRankContext pageRankContext =
          findPageRankContext(q.getQueryId(), q.getUserId(), pageRankContexts);

      assert pageRankContext != null;

      if (pageRankContext.getInDegree() == 0
          && pageRankContext.getNeighbourIdsWithWeight().size() == 0) {
        ...
        context.send(
            MessageBuilder.forAddress(
                ...
                .withCustomType(
                    Types.QUERY_PAGERANK_RESULT_TYPE,
                    new QueryPageRankResult(
                        q.getQueryId(), q.getUserId(), context.self().id(), null))
                .build());
  ...
  private void sendPrepareResponseToCoordinator(Context context, PageRankTaskWithState q) {
    context.send(
        MessageBuilder.forAddress(
                TypeName.typeNameOf("hesse.coordination", "coordinator"), q.getCoordinatorId())
            .withCustomType(
  ...
  private PageRankContext findPageRankContext(
      String queryId, String userId, ArrayList<PageRankContext> list) {
