...
/**
 * Applications need to filter the activities in [T0, Tt] to recover state This function aims to
 * receive and aggregate the processing time for filtering it will output filtering time for the
 * current query, the overall filtering time for handling different queries the number of queries
 * and the average filtering time one query takes
 */
public class BenchmarkFilterTimeFn implements StatefulFunction {
  static final TypeName TYPE_NAME =
      TypeName.typeNameOf("hesse.benchmarks", "benchmark-filter-time");
  ...
  private static final org.slf4j.Logger LOGGER =
      LoggerFactory.getLogger(BenchmarkFilterTimeFn.class);
  ...
  public CompletableFuture<Void> apply(Context context, Message message) throws Throwable {
    if (message.isLong()) {
      ...
      LOGGER.trace(
          "[BenchmarkFilterTimeFn {}] current filtering time: {}, overall time in nano seconds: {}, record number: {}, average time for each filtering in nano seconds: {}",
          context.self().id(),
          time,
          overallTime,
          recordNumber,
